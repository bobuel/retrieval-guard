...
class RetrievedDocument:
    ...
    verifier_score: float | None  # Stage 2 score (None if not verified)
...
class TwoStagePipeline:
    ...
    def _call_retriever(self, query: str) -> list[RetrievedDocument]:
        ...
        if self._retriever_type == "retrieve_protocol":
            try:
                ...
                # LlamaIndex retrievers accept a query (or QueryBundle) but
                # configure top-k on the retriever rather than per call.
                if "top_k" not in str(error):
                    raise
    ...
    def retrieve(self, query: str, top_k: int | None = None) -> list[RetrievedDocument]:
        ...
        for i, doc in enumerate(candidates):
            if self.verification_mode == "full" or self.verification_mode == "light" and doc.score < self.light_threshold:
