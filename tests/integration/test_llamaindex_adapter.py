...
class TopOneVerifier:
    """Verifier with a zero margin, so only the highest-scoring document passes."""
    rejection_threshold = 0.0
    ...
    def score_batch(self, query, candidates):
        return [1.0 - float(index) for index in range(len(candidates))]
...
class TestGuardedLlamaRetriever:
    ...
    def test_zero_margin_returns_only_top_result(self):
        """With a zero margin, only the top verifier result is returned."""
        from retrieval_guard.adapters.llamaindex import GuardedLlamaRetriever
        from retrieval_guard.verifier.pipeline import TwoStagePipeline
